...
- **分层协作多智能体架构（LCMAA）**：提出四层分离的事件驱动异步架构，解耦表示层、服务协调层、智能体协作层与知识存储层，以工作流编排（DAG）驱动八类专业化智能体协同。
- **面向教育场景的多智能体闭环**：构建“解释→决策→画像→检索→苏格拉底引导→规划→执行→学习分析”的闭环流程，使用共享 `AgentState` 实现低耦合状态传递与事件触发同步。
- **混合知识检索与多级缓存**：将图存储（NetworkX）与向量检索（HNSW/NanoVectorDB）和文档库（MongoDB）融合，设计并发检索与TTL缓存策略，实现结果合并、去重与打分的高效管线。
- **苏格拉底式自适应提问引擎**：依据回答质量与对话状态动态选择问题类型与难度，支持理解检查、深入探索与多轮连贯追问，兼容LLM与规则回退。
- **用户画像的三元组与维度映射**：融合LLM提示与规则抽取用户画像三元组，增量维护实体/关系，并映射至学习风格、知识水平等维度，支持画像摘要与个性化前缀生成。
- **决策分析与策略选择**：基于多指标启发式评分判断是否检索与处理模式（Direct/Retrieval/Socratic），并输出核心概念与优化查询策略。
- **规划与可执行行动流水线**：按复杂度与场景生成计划类型、行动序列与成功标准，执行阶段支持LLM失败时的模板回退与个性化聚合响应。
- **性能与工程优化**：采用异步I/O、并发检索、批处理与模型预热，配合多级缓存与增量同步降低端到端延迟；提供可扩展的适配器/接口以便新智能体与新存储接入。
...
（综合叙述，顶刊风格）本系统以有向无环工作流为骨架，将解释、决策、检索、引导、规划、执行、分析与画像维护编织为一条端到端的认知管线。入口由查询解释单元对用户输入进行统一规范化：在轻量预处理之后，调用提示工程驱动的大模型以获得意图、类型、关键词与模式建议；若解析失败，则回退至可解释的规则管线以稳健产出同构结构化结果。由此形成的标准化语义表示不仅降低了下游耦合，也为后续个性化与检索策略提供了充分的先验。

在此基础上，决策单元以启发式多指标评分裁决是否进入检索路径，并同步产出核心概念与多层查询。综合打分采用\(score_{total}=0.4\,s_{strong}+0.3\,s_{type}+0.3\,s_{complex}\)，当且仅当 \(score_{total}>\tau\)（\(\tau=0.5\)）且不命中“无需检索”规则时触发检索分支。概念抽取与查询生成遵循“精确—概念—多项—关键词”的层级策略，以权重化排序确保在延迟与收益之间取得稳定平衡。

知识检索单元在统一的目标选择器驱动下并发访问文档片段库、图存储与向量索引，利用内存级TTL缓存避免冗余I/O；跨源候选以去重与加权融合得到一致排序。相关性采用可解释的合成评分：\(score(x,q)=\lambda_1\,\mathrm{Jaccard}(kw(x),kw(q)) + \lambda_2\,\mathrm{len\_penalty}(x) + \lambda_3\,\mathrm{struct\_match}(x)\)。该形式在工程上具备良好的稳定性与可调性，支持在不同后端与数据分布下进行快速标定。

当系统判断直接生成无法保证学习收益或存在理解缺口时，苏格拉底引导单元介入，通过对用户响应质量与对话态势的轻量评估，选择最具教学价值的问题类型 \(t^{*}=\arg\max_{t\in\mathcal{T}} u(t\,|\,quality,state)\)。继续与否由规则 \(\mathrm{continue}=\mathbb{I}(quality<\theta_1\ \lor\ t^{*}\in\{clarify,evidence\})\ \wedge\ (rounds<R_{max})\) 决定，以控制认知负荷并保持对话张力。

规划单元在“情况评估—类型映射—行动生成—约束优化”的流水线下工作：将查询复杂度、知识可得性、参与度、进展与上下文丰富度聚合为 \(overall\_complexity=\sum_i \beta_i m_i\)，再与标准化的查询类型联合映射到计划类型，并据此生成可执行的行动序列与成功判据。计划以四元组 \(Plan=(type,\ Actions,\ SuccessCriteria,\ Alternatives)\) 表征，借助步数限制、优先级排序与顺序标注保证可落地与可回退。

执行单元以最小必要集成的方式调用生成模型与模板回退，按优先级逐项兑现行动，并在失败时快速降级以保障整体完成度。最终响应由个性化前缀与成功行动结果进行可解释聚合：\(Response=personalize(profile)\ \|\!\|\ aggregate(\{r(a)\}_{a\in Actions})\)。与此同时，系统记录细粒度的执行报告，为学习分析与后续自适应调参提供可观测信号。

学习分析单元从参与度、覆盖度、响应效果与学习价值四维构建基础质量分，并在协作语篇场景中引入连贯性、决策质量与个性化利用度的增强重加权：\(Score' = \gamma_0\,Score + \gamma_1\,coherence + \gamma_2\,decision\_quality + \gamma_3\,personalization\)。该单元进一步挖掘洞察与缺口，生成具操作性的反馈与下一步建议，形成教学闭环。

最后，用户画像单元以三元组为基本语义载体，融合LLM提示抽取与规则模板以增量维护用户知识图谱：\(T' = T \cup extract\_triples(interactions)\)，并将结构化事实映射到学习风格、知识水平与兴趣等维度，生成可复用的个性化前缀与摘要。画像与分析的耦合使系统能够在遵守稳定性与可解释性的同时，持续提升对个体差异的适配能力。
