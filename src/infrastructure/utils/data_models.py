...
class AgentState:
    ...
    socratic_guidance: Dict[str, Any] = field(default_factory=dict)  # 苏格拉底引导信息
    ...
    critique: str = ""  # 批评意见
    is_satisfactory: bool = False  # 是否通过
    revision_count: int = 0  # 修改次数

    # 元数据
    metadata: Dict[str, Any] = field(default_factory=dict)
    error_info: Optional[Dict[str, Any]] = None
    feature_flags: Dict[str, Any] = field(default_factory=dict)  # 运行时特征开关，便于消融实验
    experiment_tags: List[str] = field(default_factory=list)  # 记录实验标签，便于埋点和统计
    
    # 性能优化字段
    _cache: Dict[str, Any] = field(default_factory=dict, init=False)
    _performance_data: Dict[str, Any] = field(default_factory=dict, init=False)
    _execution_hints: Dict[str, Any] = field(default_factory=dict, init=False)
    ...
    def set_execution_hints(self, hints: Dict[str, Any]):
        """设置执行提示"""
        self._execution_hints.update(hints)
    
    def get_execution_hints(self) -> Dict[str, Any]:
        """获取执行提示"""
        return self._execution_hints.copy()
    
    def get_execution_hint(self, key: str, default: Any = None) -> Any:
        """获取特定执行提示"""
        return self._execution_hints.get(key, default)
