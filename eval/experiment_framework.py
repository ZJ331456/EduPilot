...
class ExperimentFramework:
    ...
    def _initialize_systems(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.warning(f"System initialization failed: {e}")
            return {}
    ...
    def run_comparison_experiment(self) -> Dict[str, Any]:
        ...
        for system_name, system in self.systems.items():
            ...
            system_results = self._test_system(system, system_name)
    ...
    def _test_system(self, system: Any, system_name: str) -> List[Dict[str, Any]]:
        """测试单个系统"""
        results = []
            ...
            for user_profile in self.experiment_config["user_profiles"]:
                try:
                    ...
                    if system_name == "full_multi_agent":
                        response = self._execute_full_system(system, query, user_profile)
                    elif system_name == "core_agents":
                        response = self._execute_core_system(system, query, user_profile)
                    else:  # traditional_rag
                        response = self._execute_rag_system(system, query, user_profile)
    ...
    def _execute_full_system(self, system: MultiAgentSystem, query: Dict, user_profile: Dict) -> str:
        """执行完整多智能体系统"""
        try:
            # 创建初始状态
            ...
            # 执行工作流
            final_state = system.execute_workflow(initial_state)
            
            return final_state.response if hasattr(final_state, 'response') else "No response generated"
        except Exception as e:
            return f"Error in full system: {str(e)}"
    
    def _execute_core_system(self, system: Dict, query: Dict, user_profile: Dict) -> str:
        """执行核心智能体系统"""
        try:
            # 查询解释
            interpretation = system["query_interpreter"].process(
                query["query"], user_profile, {}
            )
            
            # 直接执行
            response = system["executor"].execute(
    ...
    def _execute_rag_system(self, system: Dict, query: Dict, user_profile: Dict) -> str:
        """执行传统RAG系统"""
        try:
            # 知识检索
            retrieved_knowledge = system["retriever"].retrieve(
                query["query"], user_profile, {}
            )
            
            # 生成响应
            response = system["generator"].generate_response(
                query["query"], retrieved_knowledge, user_profile
            )
    ...
    def _save_results(self):
        ...
        try:
            ...
            # CSV格式（用于进一步分析）
            csv_file = results_dir / f"experiment_data_{timestamp}.csv"
...
if __name__ == "__main__":
    ...
    # 运行实验
    try:
        results = framework.run_comparison_experiment()
        print("\n" + "="*50)
        print("实验完成！")
        print("="*50)
        print(f"总测试数: {results['summary']['total_tests']}")
        print(f"成功测试数: {results['summary']['successful_tests']}")
        print(f"平均响应时间: {results['summary']['average_response_time']:.2f}秒")
        print(f"整体质量评分: {results['summary']['overall_quality_score']:.2f}/5.0")
